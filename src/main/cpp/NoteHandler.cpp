...
NoteHandler::NoteHandler(GlobalFaultManager *global_fault_manager) {
    ...
    launcher_config.CurrentLimits.StatorCurrentLimit = 15_A;
    ...
    tilt_configuration.Voltage.PeakForwardVoltage = 2.5_V;
    tilt_configuration.Voltage.PeakReverseVoltage = -0.6_V;
    ...
    tilt_configuration.MotionMagic.MotionMagicAcceleration = 1.3_tr_per_s_sq;
    tilt_configuration.MotionMagic.MotionMagicCruiseVelocity = 0.5_tps;
    tilt_configuration.Feedback.SensorToMechanismRatio = 50.0;
    tilt_configuration.SoftwareLimitSwitch.ForwardSoftLimitThreshold = tilt_max_position;
    tilt_configuration.SoftwareLimitSwitch.ReverseSoftLimitThreshold = tilt_park_position;
...
void NoteHandler::update_launcher_velocity_interlock() {
    ...
    frc::SmartDashboard::PutBoolean("velocity interlock", launcher_velocity_interlock);
}
...
void NoteHandler::call(bool robot_enabled, bool autonomous) {
    ...
    case LauncherState::firing:
        ...
        fire_timer_elapsed = std::chrono::steady_clock::now() > (fire_start+std::chrono::milliseconds(550));
        if(fire_timer_elapsed) {
            launcher_mode_channel.take_control(0, true);
            launcher_mode_channel.set(0, LauncherMode::idle);
        }
        break;
    }
    ...
    switch(indexing_state) {
        case IntakeIndexingState::rolling_in:
            ...
            if(!index_sensor.Get()) { 
                indexing_state = IntakeIndexingState::hold;
                indexing_mode_channel.take_control(0, true);
                indexing_mode_channel.set(0, IntakeIndexingMode::stop);
            }
            break;
